...
Data from the Soundtrap recorder and the depth sensor need to be downloaded at the end of the deployment to TWO external hard drives and mailed to two different locations.

## **Prep External Hard Drive for Data**

1.  You will need TWO external hard drives (two exact copies of the data will be made & shipped to different people). Transfer all data to ONE hard drive, then transfer that data to the second hard drive.

2.  Create a folder named after the drift, for example \'ADRIFT_009\'

3.  Create subfolder named after the drift followed by \'\_CENSOR\'. For example, \'ADRIFT_009_CENSOR\'. *This is for the data from the soundtrap---both the SUD files and the extracted files all in this folder.*
...
3.  **Plug in Soundtrap** to computer using special USB cable
    ...
    ![](images/paste-848B31C5.png){width="216"}
...
7.  **Download Data**.  De-Select \'Decompress\' and then select all and download.  The decompression takes a long time to download, and this added time can make it difficult to download all the data. It can take a long time to download the data---so be sure your computer is plugged in and that you have enough space on the drive where the data is being saved to. When the data download begins, it will tell you how many files will download (in the popup). Record this number so that you can check to be sure that all the files were downloaded to your drive.
    ...
    ![](images/paste-3CC5186B.png){width="315"}
...
## **Deployment Data- Google Drive**

1.  Go to Deployment Google Folder & fill in any final data regarding the deployment
...
2.  Copy all files to BOTH hard drives (each external hard drive should be identical).

3.  The hard drives will be mailed separately. Please box each one up so that they can be securely mailed (ideally in their original box with plastic bits; then this should be shipped in a slightly larger box with bubble wrap or other packing materials.

4.  Email kourtney.burger\@noaa.gov with the dimensions of the boxes (Length x Width x Height and weight)

5.  Kourtney will email shipping labels; these can be attached to the packages and they can be taken to a UPS/FedEx store.

6.  Please let Kourtney know if you need extra hard drives (ideally keep 2 around)
